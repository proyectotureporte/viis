...
/** Campana con avisos sin leer (→ /notificaciones) y avatar (→ /cuenta). */
export function HeaderActions() {
  const { user } = useAuth();
  const { menu } = useEmpresa();
  const unread = menu?.unreadNotifications ?? user?.unreadNotifications ?? 0;
  return (
    <View style={s.wrap}>
      <Pressable
      ...
      <Pressable onPress={() => router.push('/cuenta')} style={({ pressed }) => [s.avatar, pressed && { opacity: 0.8 }]} accessibilityRole="button" accessibilityLabel="Mi cuenta" hitSlop={6}>

...
/** POST /api/movil/v1/empresa/documentos/[id]/aprobar — Aprueba (fija vencimiento según vigencia del tipo). Sin cuerpo. */
export const POST = handler(async (request: Request, ctx: RouteCtx<'id'>) => {
  ...
  return runAction(decideDocumentAction, withFields(await formOf(request), { documentId: id, decision: 'APPROVE' }));
});

import { createTaskAction } from '@/app/(plataforma)/empresa/casos/[id]/actions';
...
/** POST /api/movil/v1/empresa/casos/[id]/tareas — Crea tarea: { assigneeId, kind: TAREA|LLAMADA|CITA|SEGUIMIENTO, title, detail?, dueAt: "YYYY-MM-DDTHH:mm" (hora Colombia) }. */
export const POST = handler(async (request: Request, ctx: RouteCtx<'id'>) => {
  ...
  return runAction(createTaskAction, withFields(await formOf(request), { opportunityId: id }));
});

import { decideDocAction } from '@/app/(plataforma)/empresa/casos/[id]/actions';
...
/** POST /api/movil/v1/empresa/casos/[id]/documentos/[docId]/revision — Decisión: { decision: "APPROVE" | "REJECT", reason? (obligatorio al rechazar, mín. 5) }. */
export const POST = handler(async (request: Request, ctx: RouteCtx<'id' | 'docId'>) => {
  ...
  return runAction(decideDocAction, withFields(await formOf(request), { opportunityId: id, documentId: docId }));
});
